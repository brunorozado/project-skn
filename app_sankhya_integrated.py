...
def analyze_sankhya_processes():
    ...
    try:
        ...
        # Gerar script Sankhya Flow (agora será BPMN XML)
        bpmn_xml_content = analyzer.generate_bpmn_xml_content(net, initial_marking, final_marking)
        ...
        response = {
            ...
            "bpmn_xml_content": bpmn_xml_content, # Adiciona o conteúdo XML ao JSON de resposta
...
@app.route("/api/download-bpmn-zip", methods=["POST"])
def download_bpmn_zip():
    """Endpoint para baixar o BPMN XML empacotado em ZIP."""
    try:
        data = request.json
        bpmn_xml_content = data.get("bpmn_xml_content")
        if not bpmn_xml_content:
            return jsonify({"status": "error", "message": "Conteúdo BPMN XML não fornecido."}), 400

        # Cria um arquivo ZIP em memória
        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            zip_file.writestr("process_model.bpmn", bpmn_xml_content)
        zip_buffer.seek(0)

        return send_file(zip_buffer, mimetype="application/zip", as_attachment=True, download_name="sankhya_flow_process.zip")

    except Exception as e:
        return jsonify({"status": "error", "message": f"Erro ao gerar ZIP do BPMN: {str(e)}"}), 500
